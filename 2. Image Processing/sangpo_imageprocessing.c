...
/******************************************************************************
  ...
  mpicc -o sangpo_imageprocessing sangpo_imageprocessing.c -lm -lglut -lGL

  To run:
  mpirun -n 4 -quiet ./sangpo_imageprocessing
...
void detect_edges(unsigned char *in, unsigned char *out) {
  ...
  for(i=0;i<n_pixels;i++) {
    int s, a; // the pixel of interest
    int m, n, o, p; // the pixels adjacent to x,y used for the calculation
    int r; // the result of calculate
    
    a = i / width;
    s = i - (width * a);

    if (s == 0 || a == 0 || s == width - 1 || a == height - 1) {
      results[i] = 0;
    } else {
      m = i + width;
      n = i - 1;
